...
当前 canonical 入口仍然是 `SynapseAuth`，因为 Provider onboarding 在工程真相里仍然属于 **owner wallet 控制面**。
...
1. `getToken()`
2. `issueProviderSecret()`
3. `listProviderSecrets()`
4. `registerProviderService()`
5. `listProviderServices()`
6. `getProviderService()`
7. `getProviderServiceStatus()`
...
const issued = await auth.issueProviderSecret({
...
const registered = await auth.registerProviderService({
...
const status = await auth.getProviderServiceStatus(registered.serviceId);
...
`wallet -> SynapseAuth -> provider control plane`

### 4.2 最小注册输入

`registerProviderService()` 对外只要求：
...
`getProviderServiceStatus(serviceId)` 返回的是控制面状态，而不是 public discover 的最终可见性承诺。
