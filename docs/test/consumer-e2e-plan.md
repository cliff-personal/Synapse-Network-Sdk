...
| 11 | 通过 discovery 选定 serviceId 调用 | `client.invoke(discoveredServiceId, payload)` → status SUCCEEDED/SETTLED |
| 12 | 通过已知 serviceId 直接调用 | `client.invoke(serviceId, payload)` → status SUCCEEDED/SETTLED |
| 13 | 费用扣除 | `invocationResult.chargedUsdc > 0` |
| 14 | 调用凭证 | `client.getInvocation(invocationId)` → 状态可查 |
| 15 | 结算后余额 | `getBalance().consumerAvailableBalance < balance_before_invoke` |
...
│  6. client.invoke(discoveredServiceId) → SUCCEEDED             │
│  7. client.invoke(serviceId) direct path → SUCCEEDED           │
│  8. client.getInvocation(id) → receipt by ID                   │
│  9. balance after < balance before invoke                      │
└─────────────────────────────────────────────────────────────────┘
```
